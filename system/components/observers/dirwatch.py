        ...
        class Handler(FileSystemEventHandler):
            def on_created(self, event):
                if not event.is_directory and Path(event.src_path).suffix == ext:
                    try:
                        logger.info(f"[Dirwatch observer] change in folder {directory} detected")
                        obs._publish_sync(
                            BrokerMessage(
                        ...
                        logger.info("Message published: %s", event.src_path)
                    except Exception:
                        logger.exception("Message publishing failed")
